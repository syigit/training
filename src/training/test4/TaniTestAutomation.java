...
import training.test4.TaniTestAutomationChromeVersion;
...
public class TaniTestAutomation {
	...
	public static void main(String[] args) throws IOException,
		...
		try {
			...
			driver.quit();
			TaniTestAutomationChromeVersion chrome=new TaniTestAutomationChromeVersion();
			chrome.main(args);
		}
	}
				...
				if (alert1Text != alert2Text) {
					...
					while (count < 60) {
						if (url.contains(driver.getCurrentUrl().toString())) {
							...
							dizi[k][k] = "Campaign Real Send Email ("
									+ castType + ")";
							dizi[k][k + 1] = "Pass";
							k++;
	...
	public static void screenShot(String methodName) throws IOException {
		...
		FileUtils.copyFile(scrFile, new File(screeenshotDirectory + "\\"
	...
	public static void createPdf() throws DocumentException,
		...
		PdfWriter.getInstance(
				document,
				new FileOutputStream("C://Users/mustafa/Desktop/"
						+ dateFormat.format(date) +"Firefox"+ ".pdf"));
		document.open();
		Paragraph title = new Paragraph("Tani Test Results", new Font(
		...
		Paragraph content = new Paragraph(
				"\nTest Platform: Selenium Web Driver\nTest Browser: Mozilla Firefox 30.0\nElapsed Time: "
						+ gecenSure / 1000 + "Seconds", new Font(
		...
		missing.setAlignment(1);
		document.add(missing);
		 DefaultPieDataset dataSet = new DefaultPieDataset();
         dataSet.setValue("Pass", k_pass);
         dataSet.setValue("Fail", k_fail);
         ...
         TextTitle titled = chart.getTitle(); 
         titled.setBorder(0, 0, 2, 0);   
         titled.setBackgroundPaint(Color.white);
         titled.setPaint(Color.DARK_GRAY);
         titled.setExpandToFitSpace(false);
    	...
    	DateFormat dateFormat2 = new SimpleDateFormat("dd/MM/yyyy hh:mm");
   		Date date2 = new Date();
   	 Paragraph subject=new Paragraph(dateFormat2.format(date2)+"\n", new Font(Font.FontFamily.SYMBOL, 8));
