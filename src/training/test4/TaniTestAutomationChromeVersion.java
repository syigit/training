...
public class TaniTestAutomationChromeVersion {
	...
	public static void main(String[] args) throws IOException,
		...
		{  System.setProperty("webdriver.chrome.driver", "C:/Users/mustafa/Desktop/chromedriver/chromedriver.exe");
		   driver=new ChromeDriver();	
			...
			log.info("Test Succeed " + "Elapsed Time: " + gecenSure/1000+" Seconds");
	...
	public static void yeniHedefListe(String companyName,String targetType,String targetSource) throws InterruptedException,
			IOException {
		try {
			if(targetType=="Gerçek Gönderim")log.info("newRealTargetList Creating");
			else log.info("newTestTargetList Creating");
			...
			try {// butona tıklama işlemi olumsuz sonuçlanırsa popup açılmamış
				...
				try {
					...
					if(targetType=="Gerçek Gönderim")log.info("newRealTargetList Created");
					else log.info("newTestTargetList Created");
					okButton.click();				
				} catch (Exception e) {
			...
			if (saveSuccess==true)
			{
				...
				WebElement alert2=searchElement(driver,".preserve-newline.margin-bottom-5.alert.alert-success");
				 ...
				 if(alert1Text!=alert2Text)
				 {
					 ...
					 WebElement acceptSend=driver.findElement(By.cssSelector("input.campaignTestApprove.uniform"));
					 acceptSend.click();
					...
					if(count>=60) log.error("Real Send Failed");
					}
				 }
				 else
	...
	IOException {
		try {
			if(castType=="Tekil Gönderim") log.info("SendCampaignSms (Single) Started");
			else log.info("SendCampaignSms (Multi) Started");
			String alert1Text = null,alert2Text=null;
			long unixTime = System.currentTimeMillis() / 1000L;
			...
			selectCompany.sendKeys(Keys.RETURN);// işyeri seçiliyor✓
			//-----------------------------------------------------------------------
			WebElement targetSelect=searchElement(driver,"#s2id_smsCampaignTestTargetList>a>.select2-chosen");
			targetSelect.click();
			targetSelect.sendKeys(Keys.ARROW_DOWN);
			targetSelect.sendKeys(Keys.RETURN); //target list seçiliyor ✓
			//-------------------------------------------------------------------------
			...
			WebElement messageContent=searchElement(driver,"#tinymce");
			messageContent.sendKeys("Mesaj icerigi"); //mesaj içeriği giriliyor✓
			//-------------------------------------------------------------------------
			driver.switchTo().defaultContent();
			WebElement titleSelect=searchElement(driver," #s2id_smsCampaignPrefix>a>.select2-chosen");
			titleSelect.click();
			titleSelect.sendKeys(Keys.RETURN); //başlık seçiliyor ✓
			//Kampanya Kaydet Kontrolü//
			WebElement saveButton=searchElement(driver,".btn.btn-lg.blue.campaignSave");
	...
	/**
	 * 
	 * @param driver o an kullandigimiz WebDriver`ı alıyor
	 * @param cssText Aradigimiz nesnenin cssSelector bilgisini alıyor
	...
	public static void screenShot(String methodName) throws IOException
	{
		...
		Date date = new Date();
		FileUtils.copyFile(scrFile, new File(screeenshotDirectory+methodName+".png"));
		log.info("Screen Captured");
		log.info("File: "+screeenshotDirectory+methodName+dateFormat.format(date)+".png");
	}
}